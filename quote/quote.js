...
function render() {
  ...
  const checked = Array.from(document.querySelectorAll(".checkbox input[type=checkbox]:checked"));
  const optionTotal = checked.reduce((sum, el) => sum + Number(el.dataset.price), 0);
  const buildTotal = 1500000 + optionTotal;
  ...
  rows.innerHTML = `<tr><td>기본 구축 (검색·AI검색 최적화 세컨 홈페이지)</td><td>${won(1500000)}</td></tr>`;
  checked.forEach((el) => {
